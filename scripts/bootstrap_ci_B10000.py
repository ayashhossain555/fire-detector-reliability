...
helps" claims need.
Writes analysis/calibration_ci_B10000/<cell>.json  (copy of bootstrap_ci.py; only the OUT folder, the B default 10000 and the temp-file prefix differ).
Usage: python analysis/bootstrap_ci_B10000.py --cell <name> [--B 10000]
       python analysis/bootstrap_ci_B10000.py --all      # every cell lacking a B=10000 CI
"""
import argparse
import contextlib
import io
...
OUT = HERE / "calibration_ci_B10000"  # B = 10000 re-run (referee p-value-resolution point, 2026-09-13); calibration_ci/ is never written
...
def run_cell(cell, B):
    ...
    for key in ("calval-dets", "test-dets"):
        ...
        if len(keep) != len(dets):
            fp = tmp / f"ci10k__{cell}__{key}.json"; fp.write_text(json.dumps(keep), encoding="utf-8"); inp[key] = str(fp)
